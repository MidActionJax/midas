...
file_name = "01_MES_Level2_Dump_20260319_103521" 
...
print("🎯 Hunting for the 2-Point DROPS (Creating the Short Target)...")
# The "Time Machine" - We look exactly 60 rows (60 seconds) into the future
book['Future_Price_60s'] = book['Mid_Price'].shift(-60)

# THE SHORT CHECKLIST TARGET:
# If the future price is 2.0 points (or more) LOWER than the current price, label it 1 (Success).
# We use <= -2.0 to represent a 2-point drop.
...
if total_seconds > 0:
    print(f"Percentage of time the market drops: {(total_jumps/total_seconds)*100:.2f}%")
print("======================================================\n")

print("💾 Saving the AI-ready dataset...")
# Save the new, much smaller, highly targeted dataset
book.to_csv("Short_ML_Ready_Data_Week1.csv")
