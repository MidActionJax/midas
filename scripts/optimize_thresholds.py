...
MODEL_FILE = os.path.join(BASE_DIR, 'models', 'midas_brain.pkl')

# Target your historical validation data (adjust the path if necessary)
DATA_DIR = os.path.join(BASE_DIR, 'data', 'lvl_2_clean')
...
def run_grid_search():
    ...
    df = pd.read_csv("Master_ML_Ready_Data_Long.csv")
    
    # --- 2.5 Inject Feature Engineering ---
    print("🔧 Reconstructing time-series features (Trend and Volatility)...")
    
    if 'Timestamp' in df.columns:
        df['Timestamp'] = pd.to_datetime(df['Timestamp'], utc=True)
        df.sort_values('Timestamp', inplace=True)
        # Reconstruct Hour if missing
        if 'Hour' not in df.columns:
            df['Hour'] = df['Timestamp'].dt.hour
            
    # Reconstruct Mid_Price if missing
    ...
    df = df[df['Mid_Price'] > df['SMA_Macro']]  # Macro Guard (Only Longs in Uptrends)
